...
export function renderNav({ activePage, cartCount }) {
  ...
  return `<a href="#main" class="skip-link">Saltar al contenido</a><div class="announcement">Un pequeño break. Un gran antojo. <span>Eso es Snackyzz.</span></div><header class="site-header"><div class="nav-shell"><a href="#" class="brand" aria-label="Snackyzz, inicio">${wordmark()}</a><nav class="nav" aria-label="Navegación principal">${links.map(([id, label]) => `<a href="#${id === "home" ? "" : id}" class="nav-link ${activePage === id ? "is-active" : ""}" ${activePage === id ? 'aria-current="page"' : ""}>${label}</a>`).join("")}</nav><div class="nav-actions"><a href="#shop" class="cart-button" aria-label="Ver carrito, ${cartCount} productos">${icon("bag")}<span>Mi carrito</span><b data-cart-count>${cartCount}</b></a><button class="menu-button icon-button" data-menu aria-label="Abrir menú" aria-expanded="false">${icon("menu")}</button></div></div><nav class="mobile-nav" aria-label="Navegación móvil" hidden>${links.map(([id, label]) => `<a href="#${id === "home" ? "" : id}">${label}</a>`).join("")}</nav></header>`;
}
export function renderFooter() {
  return `<footer class="site-footer"><div class="footer-top"><div><a href="#" class="brand">${wordmark()}</a><p>La vida se disfruta<br>un bocado a la vez.</p></div><div class="footer-links"><h2>¿Se te antoja?</h2><a href="#shop">Las cookies</a><a href="#sales">Dónde encontrarnos</a><a href="#about">Sobre Snackyzz</a></div><div class="footer-contact"><h2>Sigamos en contacto</h2><p class="social-handle">${icon("instagram")} @Snackyzz.cookies</p><a href="mailto:Snackyzz.cookies@gmail.com">${icon("mail")} Snackyzz.cookies@gmail.com</a><p>Escríbenos. Nos encantará leerte.</p></div></div><div class="footer-bottom"><span>© ${new Date().getFullYear()} Snackyzz. Todos los derechos reservados.</span><a href="#admin">Administración ${icon("arrow")}</a></div></footer>`;
}
